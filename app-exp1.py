...
st.write("Pilih kategori (bentuk) dengan rata-rata Y tertinggi dari scatterplot di bawah.")

# --- Kumpulkan semua shape dari 5 folder ---
...
if len(SHAPE_POOL) < 10:
    st.error("Jumlah total shape terlalu sedikit. Pastikan minimal 10 bentuk tersedia di folder-folder tool.")
    st.stop()

# --- Setup Eksperimen State ---
if "task_index" not in st.session_state:
    st.session_state.task_index = 0
    st.session_state.correct = 0
    st.session_state.total_tasks = 50

# --- Reset flag submit per task ---
...
# --- Pilih kategori ---
selected_label = st.selectbox("📍 Pilih kategori dengan rata-rata Y tertinggi:", [f"Kategori {i+1} ({label})" for i, label in enumerate(shape_labels)])
...
if st.button("🚀 Submit Jawaban"):
    ...
    if benar:
        ...
        st.error(f"❌ Jawaban salah. Jawaban benar adalah Kategori {true_index+1} ({shape_labels[true_index]})")
...
if st.session_state.task_index >= st.session_state.total_tasks:
    st.success(f"🎉 Eksperimen selesai! Skor akhir Anda: {st.session_state.correct} dari {st.session_state.total_tasks}")
    st.balloons()
